        ...
        switch (userChoice.trim()) {
            case '1':
                ...
                rl.question("What is the name for the order?\n", function (name) {
                    rl.question("What is your  address?\n", function (address) {
                        let items = [];
                        function askForItem() {
                            rl.question("What would you like to order? (Type 'done' when finished)\n", function (order) {
                                if (order.trim().toLowerCase() === 'done') {
                                    ...
                                    rl.question("Enter the quantity:\n",
                                        price = rl.question("enter the price\n"),
                                        if (price == "") { function (quantity) { items.push({ name: order.trim(), price: getRandomPrice(), quantity: parseInt(quantity) }});
                            askForItem();
                        },
                                        else {
                            function (quantity) {
                                items.push({ name: order.trim(), quantity: parseInt(quantity) });
                                askForItem();
                            }
