...
const db = new sqlite3.Database('data.db'); // Use a file-based SQLite database
...
const sessionMiddleware = session({
    ...
    secret: 'fnuiefbuiahguoqshfiusaunvuiqagvuobcfehhifbgyihfqabefbqehyivbghfneqgbfefuiabfduiwqgbuoashefuoqnuofvgguoqhqofq1fwqhfuoanuofbhqqi3hiorunhquonfcowqkgfwiqjrfhuifniuahfoujiowjfoijh3fauoeoiqnaffiuhguihwQUHFIBVUJNHEDJJDNUOHWSOIJ', // Replace with a strong secret key
    resave: false,
...
io.on('connection', (socket) => {
    ...
    socket.on('join room', (room) => {
        ...
        db.all('SELECT user, content, date FROM messages WHERE room = ? ORDER BY date ASC', [room], (err, rows) => {
            if (err) {
                console.error('Error retrieving chat history:', err);
            } else {
                rows.forEach(row => {
                    socket.emit('chat message', `${row.user} [${row.date}]: ${row.content}`);
                });
            }
        });
    });
        ...
        if (msg.startsWith('/')) {
            ...
            switch (command) {
                ...
                case 'leave':
                    ...
                    // Retrieve and send chat history for default room
                    db.all('SELECT user, content, date FROM messages WHERE room = ? ORDER BY date ASC', ['default'], (err, rows) => {
                        if (err) {
                            console.error('Error retrieving chat history:', err);
                        } else {
                            rows.forEach(row => {
    ...
    // Send chat history for default room when user connects
    db.all('SELECT user, content, date FROM messages WHERE room = ? ORDER BY date ASC', ['default'], (err, rows) => {
        if (err) {
            console.error('Error retrieving chat history:', err);
        } else {
